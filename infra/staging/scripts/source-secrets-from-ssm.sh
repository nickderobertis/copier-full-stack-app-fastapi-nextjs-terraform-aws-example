...
region_script="$(dirname $(realpath ${BASH_SOURCE[0]}))/region.sh"
full_app_name_script="$(dirname $(realpath ${BASH_SOURCE[0]}))/full-app-name.sh"
generated_dir="$(dirname $(dirname $(realpath ${BASH_SOURCE[0]})) )/generated"

region=$($region_script)
full_app_name=$($full_app_name_script)
source_params_script="$(dirname $(dirname $(dirname $(realpath ${BASH_SOURCE[0]})) ) )/scripts/internal/source-params.sh"
